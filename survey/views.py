...
def _survey_redirect(request, survey,
    ...
    """
    # XXX HACK
    #     IFF the survey slug is '24' then we are going to redirect them
    #     to the conference downlaods page.
    #
    if survey.slug == "24":
        return HttpResponseRedirect(reverse('about_template', kwargs = {'template': 'conference_downloads.html'}))
...
def survey_detail(request, survey_slug,
    ...
    if (hasattr(request, 'session') and
        (survey.has_answers_from(request.session.session_key) or
         (request.user.is_authenticated() and survey.has_answers_from_user(request.user))) and
    ...
    return delete_object(request, object_id=question_id,
        **{"model":Question,
         ...
         'extra_context': {'title': _('Delete question'),
                           'survey' : question.survey, 'group_slug' : group_slug }
        })
